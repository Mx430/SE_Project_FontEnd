<template>
    <div class="home-index">
        ...
        <div class="page-wrapper">
            <a-row :gutter="24">
                <!-- 项目列表 -->
                <a-col :xl="16" :lg="24" :md="24" :sm="24" :xs="24" :span="16">
                    <a-card class="project-list" :loading="loading"
                        style="margin-bottom: 24px;background-color: #F1E5D8; border-style: solid; border-width: 10px; border-color: #85B91B; border-radius: 25px;"
                        :bordered="false" title="进行中的项目" :body-style="{ padding: 0}">
                        <router-link to="/project/list/my" slot="extra">全部项目</router-link>
                        <div>
                            <a-card-grid style="background-color: #FF4E54; border-radius: 20px;" class="project-card-grid" :key="i"
                                v-for="(item, i) in projectList">
                                <a-card style="background-color: #FF4E54" :bordered="false" :body-style="{ padding: 0 }"
                                    @click="routerLink('/project/space/task/' + item.code)">
                                    <a-card-meta>
                                        <div slot="title" class="card-title">
                                            <a-avatar size="small" :src="item.cover" />
                                            <router-link :to="'/project/space/task/' + item.code">
                                                <a-icon type="star" theme="filled" style="color: #ffaf38;margin-right: 6px;"
                                                    v-show="item.collected" />{{ item.name }}
                                            </router-link>
                                        </div>
...
<!--                     <a-card class="tasks-list" style="margin-bottom: 24px"
                ...
                <a-col
                        ...
                        :span="8">
                    <a-card title="日程">
                        <router-link to = "/calendar" slot = "extra">全部日程</router-link>
                        <div class="calendar">
                            <a-Calendar :fullscreen = Boolean(0) onPanelChange={onPanelChange} />
                        </div> 
                    </a-card>
                </a-col>
            </a-row>
...
import { fastKey } from 'core-js/modules/_meta';
